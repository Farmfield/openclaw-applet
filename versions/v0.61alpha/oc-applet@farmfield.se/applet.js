...
MyApplet.prototype = {
    ...
    _setupMenu: function() {
        try {
            ...
            this.menu.box.set_width(200);

            // Title at top (centered) - add directly to menu box
            let titleLabel = new St.Label({ text: "OC Control", style: "font-size: 14px; font-weight: bold;" });
            ...
            this.modelsSubmenu = new PopupMenu.PopupSubMenuMenuItem("OC Models", true);
            ...
            // Separator between OC Models and OC Start
            ...
            // Separator between OC Restart and OC Dashboard
            this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
            ...
            // Separator between OC Dashboard and OC Json
            this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
            ...
            // Separator between OC Folder and OC Doctor
            this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
            ...
            // Separator after OC Doctor + Footer
            this.menu.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());
            
            // Footer: Settings button as a menu item, version as another
            let settingsItem = new PopupMenu.PopupIconMenuItem(
                "Settings",
                "settings-icon",
                St.IconType.FULLCOLOR
            );
            // Make it look smaller like footer text
            settingsItem.label.set_style("font-size: 9px;");
            settingsItem.connect('activate', Lang.bind(this, this._showSettings));
            this.menu.addMenuItem(settingsItem);
            
            // Version text (right-aligned, added directly to menu box)
            let versionLabel = new St.Label({ text: "0.61a - ByFarmfield - 2026", style: "font-size: 9px; color: #888888;" });
        ...
        if (models.length === 0) {
            ...
            for (let i = 0; i < models.length; i++) {
                ...
                mItem.connect('activate', Lang.bind(this, function() {
                    let patchCmd = 'openclaw sessions patch agent:main:main --model ' + model_id;
                    Util.spawnCommandLine("bash -c '" + patchCmd + "'");
                    Main.notify("OC Models", "Switched to " + name);
                    this.menu.close();
                }));
    ...
    on_applet_clicked: function(event) {
        // Refresh models each time menu opens
        this._refreshModelsMenu();
        this.menu.toggle();
    },
